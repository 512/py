...
accounting spreadsheet (an OpenOffice Calc spreadsheet)
...
def guess_code(desc):
    ...
    d = "D" # dining
    t = "T" # transportation
    if desc.count("WHOLE FOOD") > 0:
        code = g
    elif desc.count("WHOLEFDS") > 0:
        code = g
    elif desc.count("TOM THUMB STORE") > 0:
        code = g
...
if __name__ == "__main__":
    ...
    # read file
    reader = csv.reader(open(op.inputfile,'rb'), delimiter=',', quotechar='"')
    lines = []
    for row in reader:
        # skip blank lines
        if len(row) == 0:
